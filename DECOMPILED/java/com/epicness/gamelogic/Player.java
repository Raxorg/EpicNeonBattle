...
public class Player {
    ...
    private Team team;
    private int tech = 3;
    private ArrayList<Cell> territory = new ArrayList();
    ...
    public void buy(Placeable placeable, Cell cell) {
        if (!cell.isBlocked()) {
            int r;
            int c;
            Cell currentCell;
    ...
    private void wallDestroyed(int row, int column) {
        ...
        for (r = -1; r <= 1; r++) {
            for (c = -1; c <= 1; c++) {
                Cell currentCell = GameMaster.getInstance().getMap().getCell(row + r, column + c);
                    ...
                    if (currentCell != null && (((currentCell.getContent() instanceof Wall) || (currentCell.getContent() instanceof Base)) && currentCell.getContent().getOwner().getTeam() == this.team)) {
                        for (int r2 = -1; r2 <= 1; r2++) {
                            for (int c2 = -1; c2 <= 1; c2++) {
                                Cell currentCell2 = GameMaster.getInstance().getMap().getCell(currentCell.getRow() + r2, currentCell.getColumn() + c2);
