...
public class DoingStuffProcessor extends InputAdapter {
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        if (!GameMaster.getInstance().getActionObserver().getAction().equals(Action.OBSERVING)) {
            Cell touchedCell = calculateCellTouched(screenX, screenY);
            if (touchedCell != null) {
    ...
    public boolean touchUp(int screenX, int screenY, int pointer, int button) {
        if (GameMaster.getInstance().getActionObserver().getAction().equals(Action.BUILDING) && calculateCellTouched(screenX, screenY) == this.currentCell && this.currentCell != null) {
    ...
    private Cell calculateCellTouched(int screenX, int screenY) {
        int r = screenY;
        int c = screenX;
