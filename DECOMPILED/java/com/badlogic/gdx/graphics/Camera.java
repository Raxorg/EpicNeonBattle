...
public abstract class Camera {
    ...
    public void lookAt(float x, float y, float z) {
        ...
        if (!this.tmpVec.isZero()) {
            ...
            if (Math.abs(dot - 1.0f) < MathUtils.nanoToSec) {
                this.up.set(this.direction).scl(-1.0f);
            } else if (Math.abs(dot + 1.0f) < MathUtils.nanoToSec) {
    ...
    public Vector3 unproject(Vector3 screenCoords, float viewportX, float viewportY, float viewportWidth, float viewportHeight) {
        ...
        float y = ((((float) Gdx.graphics.getHeight()) - screenCoords.y) - 1.0f) - viewportY;
