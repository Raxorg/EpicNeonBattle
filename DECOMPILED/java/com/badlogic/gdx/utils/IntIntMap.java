...
public class IntIntMap implements Iterable<Entry> {
    ...
    private static class MapIterator {
        ...
        public void remove() {
            if (this.currentIndex == INDEX_ZERO && this.map.hasZeroValue) {
                ...
                this.map.keyTable[this.currentIndex] = IntIntMap.EMPTY;
            }
            this.currentIndex = INDEX_ILLEGAL;
            IntIntMap intIntMap = this.map;
            intIntMap.size += INDEX_ZERO;
        }
    }
    ...
    public static class Entries extends MapIterator implements Iterable<Entry>, Iterator<Entry> {
        ...
        public Entry next() {
            if (!this.hasNext) {
                ...
                int[] keyTable = this.map.keyTable;
                if (this.nextIndex == -1) {
                    this.entry.key = IntIntMap.EMPTY;
    ...
    public static class Keys extends MapIterator {
        ...
        public int next() {
            if (!this.hasNext) {
                ...
                int key = this.nextIndex == -1 ? IntIntMap.EMPTY : this.map.keyTable[this.nextIndex];
                this.currentIndex = this.nextIndex;
                findNextIndex();
                return key;
            } else {
    ...
    public int hashCode() {
        int h = EMPTY;
        if (this.hasZeroValue) {
            h = EMPTY + Float.floatToIntBits((float) this.zeroValue);
        }
        int[] keyTable = this.keyTable;
        int[] valueTable = this.valueTable;
    ...
    public boolean equals(Object obj) {
        ...
        for (int i = EMPTY; i < n; i++) {
            int key = keyTable[i];
            if (key != 0) {
                int otherValue = other.get(key, EMPTY);
